...
<?php endif; ?>
<?php if ( '_startseite' === $this->pageclass_sfx || '_aktuelles' === $this->pageclass_sfx ) {

	$link = JRoute::_(ContentHelperRoute::getArticleRoute($this->item->slug, 339, $this->item->language));
	echo '<a href="' . $link . '" data-id="'.$this->item->catid.'">';

	preg_match('/<h4>(.*)<\/h4>/i', $this->item->introtext, $matches);
	if (array_key_exists ( 1 , $matches ) ) {
		$headline = strip_tags ( preg_replace("/<br\W*?\/>/", " ", $matches[1]) );

		echo '_startseite' === $this->pageclass_sfx ? '<h4>' . $headline . '</h4>' : '<h3>' . $headline . '</h3>' ;
	}

	preg_match('/(<img[^>]+>)/i', $this->item->introtext, $matches);
	if (array_key_exists ( 1 , $matches ) ) {
		echo $matches[1];
	}
	...
	preg_match('/<div class="presse_datum">(.+?)<\/div>/i', $this->item->introtext, $matches);
	if (array_key_exists ( 1 , $matches ) ) {
		echo strip_tags ( preg_replace("/<br\W*?\/>/", " ", $matches[1]) );
	}

	preg_match('/<div class="presse_teaser">(.+?)<\/div>/i', $this->item->introtext, $matches);
	if (array_key_exists ( 1 , $matches ) ) {
		echo ' – ' . strip_tags ( preg_replace("/<br\W*?\/>/", " ", $matches[1]) );
	}
